...
class ReferenceExecutor implements ExecutorImplementation
{
	...
	public function doExecuteSubscribe(): Promise
	{
		...
		try {
			if (!$adapter instanceof AsyncAdapter) {
				throw new RuntimeException('When handling subscriptions, an AsyncAdapter is required. ' . Utils::printSafe($adapter) . ' does not implement AsyncAdapter.');
			}

			$type = $this->getOperationRootType($this->exeContext->schema, $this->exeContext->operation);
			$fields = $this->collectFields($type, $this->exeContext->operation->selectionSet, new ArrayObject(), new ArrayObject())
			               ->getArrayCopy();

			$responseName = array_key_first($fields);
			$fieldNodes = $fields[$responseName];

			$fieldName = $fieldNodes[0]->name->value;
			$fieldDef = $this->getFieldDef($this->exeContext->schema, $type, $fieldName);

			if (!$fieldDef) {
				throw new Error("The subscription field \"${fieldName}\" is not defined.", $fieldNodes);
			}

			$path = [$responseName, $type->name];
			$info = new ResolveInfo(
				...
				$this->exeContext->fragments,
				$this->exeContext->rootValue,
				$this->exeContext->operation,
			...
			try {
				...
				$resolveFn = $fieldDef->subscribeFn ?? $this->exeContext->fieldResolver;

				$stream = $resolveFn($this->exeContext->rootValue, $args, $contextValue, $info);

				if (!$stream instanceof Promise && !$adapter->isThenable($stream)) {
					$stream = $adapter->createFulfilled($stream);
				} elseif (!$stream instanceof Promise && $adapter->isThenable($stream)) {
					$stream = $adapter->convertThenable($stream);
				}

				$stream = $stream->then(function ($stream) use ($adapter) {
					if (!$this->isAsyncIterator($stream)) {
						throw new Error('Subscription field must return Async Iterable. Received: ' . Utils::printSafe($stream));
					}

					return $adapter->convertAsyncIterable($stream);
				}, function (Throwable $e) use ($path, $fieldNodes) {
					$this->exeContext->addError(Error::createLocatedError($e, $fieldNodes, $path));
		...
		$result = $stream ?? $this->buildResponse(null);

		if (!$result instanceof Promise && $adapter->isThenable($result)) {
			$result = $adapter->convertThenable($result);
		}

		if (!$result instanceof Promise) {
			$result = $adapter->createFulfilled($result);
		}
	...
	protected function resolveField(ObjectType $parentType, $rootValue, ArrayObject $fieldNodes, array $path)
	{
		...
		} else if ($parentType->resolveFieldFn !== null) {
	 ...
	 * @return bool
	 */
	protected function isAsyncIterator($value): bool
	{
		$adapter = $this->exeContext->promiseAdapter;

		if ($adapter instanceof AsyncAdapter) {
			return $value instanceof AsyncIterator || $adapter->isAsyncIterable($value);
		}

		return $value instanceof AsyncIterator;
	}
	...
	)
	{
		...
		} else if (is_callable($typeCandidate)) {
